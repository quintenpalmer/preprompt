...
func getPersists(persistRepr interface{}) (PersistList, error) {
	...
	if !ok {
		return nil, Newpperror("Could not load persist array from json")
	}
	for _, persisti := range persistArray {
		persistRepr, ok := persisti.(map[string]interface{})
		if !ok {
			return nil, Newpperror("Could not load persist from json")
		}
...
func getPersistFromType(persistRepr map[string]interface{}) (Persist, error) {
	...
	switch persistType {
	case "Timer":
		return getPersistTimer(persistRepr)
	case "NoPersist":
		return getNoPersist(persistRepr)
	default:
...
func getPersistTimer(persistRepr map[string]interface{}) (Persist, error) {
	...
	if !ok {
		return nil, Newpperror("Could not load instant persist params from json")
	}
	amountFloat, ok := params["duration"].(float64)
	amount := int(amountFloat)
	if !ok {
		return nil, Newpperror("Could not load instant persist param amount from json")
	}
	pt := new(persistTimer)
