...
func getInstants(instantRepr interface{}) (InstantList, error) {
	...
	if ! ok { return nil, Newpperror("Could not load instant array from json") }
	for _, instanti := range instantArray {
		instantRepr, ok := instanti.(map[string]interface{})
		if ! ok { return nil, Newpperror("Could not load instant from json") }
...
func getInstantEffectFromType(instantEffectRepr map[string]interface{}) ([]InstantEffect, error) {
	...
	switch effectType {
		case "DirectDamage" : return getDirectDamageHelper(instantEffectRepr)
		case "NoInstant" : return getDoNothing()
		default : return nil, Newpperror("Invalid Instant Effect Type found : " + effectType)
	}
...
func getInstantCondFromType(instantCondRepr map[string]interface{}) ([]InstantCond, error) {
	...
	if ! ok { return nil, Newpperror("Could not load instant effect type from json") }
	switch effectType {
		case "AlwaysValid" : return getAlwaysValid()
		default : return nil, Newpperror("Invalid Instant Effect Type found : " + effectType)
	}
	return nil, Newpperror("Invalid Instant Effect Type found : " + effectType)
}
